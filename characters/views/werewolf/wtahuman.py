    ...
    def __init__(self, *args, character=None, **kwargs):
        ...
        if character:
            self.fields["template"].queryset = CharacterTemplate.objects.filter(
                gameline="wta", character_type="werewolf", is_public=True, status="App"
            ).order_by("name")
...
class WtAHumanExtrasView(ChargenStepMixin, SpecialUserMixin, UpdateView):
    model = WtAHuman
    fields = [
        "date_of_birth",
        "apparent_age",
...
class WtAHumanLanguagesView(ChargenStepMixin, EditPermissionMixin, FormView):
...
class WtAHumanSpecialtiesView(ChargenStepMixin, EditPermissionMixin, FormView):
    form_class = SpecialtiesForm
    template_name = "characters/werewolf/wtahuman/chargen.html"

    def get_object(self):
        """Return the WtAHuman object for permission checking."""
        if not hasattr(self, "object") or self.object is None:
            self.object = WtAHuman.objects.get(id=self.kwargs["pk"])
        return self.object
