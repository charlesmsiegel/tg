"""
Enhanced Contact/NPC creation form that supports all character types with their basics.
...
class ContactEnhancedForm(forms.Form):
    """
    ...
    rank = forms.IntegerField(
        ...
        help_text="Contact background rating (0-5)"
    )
    ...
    note = forms.CharField(
        widget=forms.Textarea(attrs={"placeholder": "Additional notes about this contact", "rows": 4}),
        label="Notes",
        required=False,
    )
    ...
    def save(self, commit=True):
        ...
        char_class = self.ALLY_CLASSES[contact_type]
        ...
        if contact_type not in ["werewolf", "changeling", "fera"]:
        ...
        if contact_type == "vampire" and self.cleaned_data.get("clan_name"):
            specific_info.append(f"Clan: {self.cleaned_data['clan_name']}")
        if contact_type == "vampire" and self.cleaned_data.get("sect_name"):
            specific_info.append(f"Sect: {self.cleaned_data['sect_name']}")

        if contact_type in ["werewolf", "fera"] and self.cleaned_data.get("breed_name"):
            specific_info.append(f"Breed: {self.cleaned_data['breed_name']}")
        if contact_type == "werewolf" and self.cleaned_data.get("auspice_name"):
            specific_info.append(f"Auspice: {self.cleaned_data['auspice_name']}")
        if contact_type in ["werewolf", "kinfolk"] and self.cleaned_data.get("tribe_name"):
            specific_info.append(f"Tribe: {self.cleaned_data['tribe_name']}")

        if contact_type == "mage" and self.cleaned_data.get("affiliation_name"):
            specific_info.append(f"Affiliation: {self.cleaned_data['affiliation_name']}")

        if contact_type == "wraith" and self.cleaned_data.get("guild_name"):
            specific_info.append(f"Guild: {self.cleaned_data['guild_name']}")

        if contact_type == "changeling":
        ...
        if contact_type == "demon" and self.cleaned_data.get("house_name"):
            specific_info.append(f"House: {self.cleaned_data['house_name']}")

        if contact_type == "fera" and self.cleaned_data.get("fera_type_name"):
