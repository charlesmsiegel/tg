"""
Enhanced Ally/NPC creation form that supports all character types with their basics.
...
class AllyEnhancedForm(forms.Form):
    """
    ...
    rank = forms.IntegerField(
        ...
        initial=1,
        label="Ally Rating",
        help_text="Ally background rating (0-5)"
    )
    ...
    note = forms.CharField(
        widget=forms.Textarea(attrs={"placeholder": "Additional notes about this ally", "rows": 4}),
        label="Notes",
        required=False,
    )
    ...
    def save(self, commit=True):
        ...
        """
        ally_type = self.cleaned_data["ally_type"]
        char_class = self.ALLY_CLASSES[ally_type]
        ...
        if ally_type not in ["werewolf", "changeling", "fera"]:
        ...
        if ally_type == "vampire" and self.cleaned_data.get("clan_name"):
            specific_info.append(f"Clan: {self.cleaned_data['clan_name']}")
        if ally_type == "vampire" and self.cleaned_data.get("sect_name"):
            specific_info.append(f"Sect: {self.cleaned_data['sect_name']}")

        if ally_type in ["werewolf", "fera"] and self.cleaned_data.get("breed_name"):
            specific_info.append(f"Breed: {self.cleaned_data['breed_name']}")
        if ally_type == "werewolf" and self.cleaned_data.get("auspice_name"):
            specific_info.append(f"Auspice: {self.cleaned_data['auspice_name']}")
        if ally_type in ["werewolf", "kinfolk"] and self.cleaned_data.get("tribe_name"):
            specific_info.append(f"Tribe: {self.cleaned_data['tribe_name']}")

        if ally_type == "mage" and self.cleaned_data.get("affiliation_name"):
            specific_info.append(f"Affiliation: {self.cleaned_data['affiliation_name']}")

        if ally_type == "wraith" and self.cleaned_data.get("guild_name"):
            specific_info.append(f"Guild: {self.cleaned_data['guild_name']}")

        if ally_type == "changeling":
        ...
        if ally_type == "demon" and self.cleaned_data.get("house_name"):
            specific_info.append(f"House: {self.cleaned_data['house_name']}")

        if ally_type == "fera" and self.cleaned_data.get("fera_type_name"):
