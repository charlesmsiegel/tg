...
class FollowerForm(forms.Form):
    ...
    follower_type = forms.ChoiceField(choices=FOLLOWER_TYPE_CHOICES, label="Follower Type")
    name = forms.CharField(
        max_length=100,
        label="Name",
        widget=forms.TextInput(attrs={"placeholder": "Name"}),
    )
    rank = forms.IntegerField(min_value=0, max_value=5, initial=1, label="Followers Rating")
    ...
    note = forms.CharField(
        widget=forms.Textarea(attrs={"placeholder": "Describe your follower and their devotion", "rows": 4}),
        label="Note"
    )
    ...
    def save(self, commit=True):
        ...
        obj = self.FOLLOWER_CLASSES[self.cleaned_data["follower_type"]].objects.create(
