...
class MentorForm(forms.Form):
    ...
    mentor_type = forms.ChoiceField(choices=MENTOR_TYPE_CHOICES, label="Mentor Type")
    name = forms.CharField(
        max_length=100,
        label="Name",
        widget=forms.TextInput(attrs={"placeholder": "Name"}),
    )
    rank = forms.IntegerField(min_value=0, max_value=5, initial=1, label="Mentor Rating")
    ...
    note = forms.CharField(
        widget=forms.Textarea(attrs={"placeholder": "Describe your mentor's expertise and how they assist you", "rows": 4}),
        label="Note"
    )
    ...
    def save(self, commit=True):
        ...
        obj = self.MENTOR_CLASSES[self.cleaned_data["mentor_type"]].objects.create(
